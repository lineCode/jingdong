...
def place_order():
    while True:
        ...
        try:
            ...
            try:
                resp = requests.get('http://op.ykjd.com/JDTrainOpen/getTaskForJD?order_src=app')
                train = resp.json()
            ...
            if partner_order_id:
                url = 'http://op.ykjd.com/JDTrainOpen/CallBackForMJD?order_id=%s&success=false&order_src=app&msg=%s' % (
